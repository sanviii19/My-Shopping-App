   ...
   const getOrderPaymentDetails = async (orderId) => {
    ...
    return (
        <div className="p-4"> 
            ...
            <div>
                {
                    orders?.map(({_id, streetAddress, createdAt, products, user, contactNumbers, orderStatus, paymentStatus, paymentDetails}) => {
                        return <div key={_id} className="border p-4 mb-4 rounded">
                            <p>User Id: {_id}</p>
                            <p>Address: {streetAddress}</p>
                            <p>Created At: {format(createdAt, "PPpp")}</p>
                            <p>User Email: {user?.email}</p>
                            <p>Contact Numbers: {contactNumbers.join(", ")}</p>
                            <p className="py-4">Order Status: {orderStatus}</p>
                            <p>Payment Status: {paymentStatus}</p>
                            <p>Payment Details: {JSON.stringify(paymentDetails, null, 2)}</p>
                            <Button onClick={getOrderPaymentDetails}>Refresh</Button>
                            <div>
